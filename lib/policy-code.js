/**
 * PR policy as code: parser/validator for repository policy
 * and evaluation against merge readiness.
 */
...
export function parsePolicy(text) {
  try {
    ...
    if (!recognized && String(text || '').trim()) {
      return { ok: false, error: 'Could not parse policy: expected fields version/requireApproval/requiredChecks/protectedPaths' }
    }
  ...
  for (const rule of policy.protectedPaths || []) {
    ...
    for (const file of changedFiles) {
      if (re.test(String(file)) && rule.requiresReviewer) {
        violations.push(`${file} matches protected path ${rule.path} — dedicated reviewer required`)
      }
    }
  }
