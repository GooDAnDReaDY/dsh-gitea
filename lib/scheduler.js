/**
 * Scheduled project checks: host-level scheduler for recurring read-only
 * checks with history. Write operations require explicit approval.
 */
...
export function nextRunAt(schedule = 'daily', now = new Date()) {
  ...
  else next.setUTCDate(next.getUTCDate() + 1) // daily by default
