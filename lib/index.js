...
const TOOL_DEFS = [
  ...
  },
  {
    ...
    description: 'Return a worktree path the agent can use; does not pin Settings to one repo.',
    parameters: {
...
export function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      let cwd = ''
      try {
        cwd = new URL(req.url, 'http://dsh.local').searchParams.get('cwd') || ''
      } catch { cwd = '' }
      cwd = String(cwd).trim()
      writeJson(res, 200, await buildGitSnapshot({ repoDir: cwd, execFile: execFileAsync }))
    },
    ...
    ctx.tools.register(
      defineTool({
        ...
        execute: async (args, exec) => {
          const cwd = args?.path || exec?.cwd || process.cwd()
