...
import { buildGitSnapshot } from './git-local.js'
...
export const Config = z.object({
  ...
  repoDir: z.string().default('')
    .description('Absolute path to the git working copy used by worktree tools and the header UI.'),
})
...
const TOOL_DEFS = [
  ...
  },
  {
    ...
    parameters: {
      path: { type: 'string', description: 'Git working copy. Falls back to Settings repoDir.' },
    },
  },
  {
    ...
    parameters: {
      ...
      path: { type: 'string', description: 'Main working copy. Falls back to Settings repoDir.' },
    },
  },
  {
    name: 'gitea_worktree_use',
    description: 'Set worktreePath as the current working copy in Settings.',
    parameters: {
...
export function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const cfg = live()
      writeJson(res, 200, await buildGitSnapshot({ repoDir: cfg.repoDir, execFile: execFileAsync }))
    },
    ...
    ctx.tools.register(
      defineTool({
        ...
        execute: async (args, exec) => {
          const cfg = live()
          const cwd = args?.path || cfg.repoDir || exec?.cwd || process.cwd()
          ...
          if (def.name === 'gitea_worktree_use' && result?.ok && result.data?.path && settingsScope) {
            await settingsScope.update({ ...cfg, repoDir: result.data.path })
          }
          return result
        },
      }),
