...
import { rememberSessionGitDirs, resolveSessionGitDir, sessionIdsFromExec, sessionCwdFromExec, repoCwdFromTool, pinDirFromTool, commandFromToolArgs, gitDirHintFromBashCommand } from './session-git.js'
...
export function apply(ctx, config) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      const repoDir = resolveSessionGitDir({ cwd, sessionId })
    ...
    ctx.tools.register(
      defineTool({
        ...
        execute: async (args, exec) => {
          ...
          const pin = pinDirFromTool({ args, result, sessionCwd })
          if (result?.ok && pin && await isGitDir(pin, execFileAsync)) {
            rememberSessionGitDirs(sessionIdsFromExec(exec), pin)
          }
          return result
        },
      }),
  ...
  ctx.on('tools/execute', async (exec, next) => {
    ...
    try {
      if (result && result.isError === false) {
        ...
        const sessionCwd = sessionCwdFromExec(exec)
        const pin = gitDirHintFromBashCommand(commandFromToolArgs(args), sessionCwd)
          || pinDirFromTool({ args, result: { ok: true, data: args }, sessionCwd: '' })
        if (pin && await isGitDir(pin, execFileAsync)) {
          rememberSessionGitDirs(sessionIdsFromExec(exec), pin)
        }
      }
