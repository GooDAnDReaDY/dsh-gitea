...
export const Config = z.object({
  ...
  webhookSecret: z.string().default('')
    .description('Secret for verifying X-Gitea-Signature on POST /dsh-gitea/webhook. Empty disables verification.'),
...
const TOOL_DEFS = [
  ...
  {
    name: 'gitea_release_now',
    description: 'One-command release plan: changelog from merged PRs, suggested semver bump, steps. Dry-run only; actual npm/GitHub publish stays with the agent workflow.',
    parameters: {
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  ...
  {
    name: 'gitea_sprint_plan',
    description: 'Plan a sprint from status/ready issues: ordered by priority, suggests a milestone. Read-only plan.',
    parameters: {
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  ...
  {
    name: 'gitea_auto_actions',
    description: 'Auto-actions by issue labels (e.g. type/security -> checklist comment + priority label). Dry-run by default; confirm: true applies.',
    parameters: {
      number: { type: 'number', required: true, description: 'Issue number.' },
      confirm: { type: 'boolean', description: 'Set true to apply (dry-run otherwise).' },
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  },
  ...
  {
    name: 'gitea_dep_watch',
    description: 'Scan repository dependencies (package.json) and produce prioritized recommendations. Read-only, never updates or creates issues.',
    parameters: {
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  ...
  {
    name: 'gitea_digest_delivery',
    description: 'Deliver a triage/health digest to an external webhook (Telegram/task inbox). Dry-run by default; audit log returned.',
    parameters: {
      kind: { type: 'string', description: 'triage or health.' },
      text: { type: 'string', description: 'Digest text to deliver (overrides kind formatting).' },
      target: { type: 'string', required: true, description: 'Webhook URL.' },
  ...
  {
    name: 'gitea_ci_status',
    description: 'List Gitea Actions runs for a repository (optionally by branch/commit SHA), with status and URL.',
    parameters: {
  ...
  {
    name: 'gitea_ci_jobs',
    description: 'List jobs of a Gitea Actions run and their logs (capped). Read-only.',
    parameters: {
      run_id: { type: 'number', required: true, description: 'Actions run id.' },
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  },
  {
    name: 'gitea_repo_create_org',
  ...
  {
    name: 'gitea_repo_branch_create',
    description: 'Create a branch in a repository.',
    parameters: {
      branch_name: { type: 'string', required: true, description: 'New branch name.' },
      ref: { type: 'string', description: 'Source ref (default main).' },
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  },
  {
    name: 'gitea_repo_branch_delete',
    description: 'Delete a branch. Requires confirm: true (boolean).',
    parameters: {
  ...
  {
    name: 'gitea_repo_tag_create',
    description: 'Create a tag in a repository.',
    parameters: {
      tag_name: { type: 'string', required: true, description: 'Tag name.' },
      target: { type: 'string', description: 'Target ref/SHA (default main).' },
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  },
  {
    name: 'gitea_repo_tag_delete',
    description: 'Delete a tag. Requires confirm: true (boolean).',
    parameters: {
  ...
  },
  {
    ...
    description: 'Update or close a milestone.',
    parameters: {
      milestone_id: { type: 'number', required: true, description: 'Milestone id.' },
  ...
  },
  {
    ...
    description: 'Delete a milestone. Requires confirm: true (boolean).',
    parameters: {
      milestone_id: { type: 'number', required: true, description: 'Milestone id.' },
      confirm: { type: 'boolean', description: 'Must be true to delete.' },
      owner: { type: 'string', description: 'Repository owner.' },
  ...
  {
    name: 'gitea_wiki_page',
    description: 'Get the content of a wiki page.',
    parameters: {
      pageName: { type: 'string', required: true, description: 'Wiki page name.' },
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  },
  {
    name: 'gitea_release_update',
  ...
  },
  {
    ...
    parameters: {
      ...
      type: { type: 'string', description: 'Webhook type (default gitea).' },
      events: { type: 'array', items: { type: 'string' }, description: 'Events to receive.' },
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  },
  {
    name: 'gitea_webhook_delete',
    description: 'Delete a webhook. Requires confirm: true (boolean).',
    parameters: {
      hook_id: { type: 'number', required: true, description: 'Webhook id.' },
      confirm: { type: 'boolean', description: 'Must be true to delete.' },
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  ...
  {
    name: 'gitea_ci_rerun',
    description: 'Rerun a failed Gitea Actions job. Requires confirm: true (boolean).',
    parameters: {
      job_id: { type: 'number', required: true, description: 'Job id.' },
      confirm: { type: 'boolean', description: 'Must be true to rerun.' },
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
    },
  },
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        const secret = live().webhookSecret
    ...
    if (live().bgSchedulerEnabled) {
      scheduler.onTick(async () => {
        ...
        if (digest?.ok && cfg.bgSchedulerWebhook) {
          ...
          const { formatDigest } = await import('./digest-delivery.js')
          const text = formatDigest('triage', digest.data)
          await deliverDigest({ target: cfg.bgSchedulerWebhook, text, dryRun: false }, {})
        }
        return digest
      })
      scheduler.start()
