...
import { rememberSessionGitDir, resolveSessionGitDir } from './session-git.js'
    ...
    ctx.tools.register(
      defineTool({
        ...
        execute: async (args, exec) => {
          ...
          const sessionCwd = exec?.cwd || exec?.agent?.session?.cwd || ''
          const pinned = String(args?.worktreePath || args?.path || '').trim()
          const cwd = pinned || sessionCwd
          ...
          const pin = String(args?.worktreePath || '').trim() || sessionCwd
          if (result?.ok && pin && await isGitDir(pin, execFileAsync)) {
            rememberSessionGitDir(sessionId, pin)
          }
          return result
        },
      }),
