...
export async function applyBatch(args = {}, deps = {}) {
  ...
  for (const issue of selected) {
    ...
    if (args.label) {
      const labelRes = await deps.client.setIssueLabels(args.owner, args.repo, issue.number, Array.isArray(args.label) ? args.label : [args.label]).catch((e) => ({ ok: false, error: String(e) }))
