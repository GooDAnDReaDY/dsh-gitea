      ...
      const save = async () => {
        ...
        try {
          if (scope && typeof scope.set === 'function') {
            const broken = []
            try { await scope.set('baseUrl', draft.baseUrl) } catch (e) { broken.push('baseUrl: ' + (e?.message || e)) }
            try { await scope.set('tokenEnv', draft.tokenEnv) } catch (e) { broken.push('tokenEnv: ' + (e?.message || e)) }
