  ...
  factory: (require) => {
    ...
    function GitStrip(props) {
      ...
      React.useEffect(() => {
        let alive = true
        const load = () => {
          if (!cwd) { setSnap({ ok: false, error: 'No session workspace' }); return }
          fetch('/dsh-gitea/git-status?cwd=' + encodeURIComponent(cwd), { cache: 'no-store' })
            .then((res) => res.json())
      ...
      if (!snap || !snap.ok) {
        return React.createElement('button', {
          ...
          title: (snap && snap.error) || 'Open a session in a git workspace',
        }, 'git')
      }
