...
export async function buildTriageDigest(args = {}, deps = {}, now = new Date()) {
  ...
  // PRs without approved review (parallelized)
  const reviewChecks = await mapConcurrent(pulls, concurrency, async (pr) => {
    ...
    return { pr, hasApproved }
  })

  const pullRequestsNoReview = reviewChecks
    .filter((r) => !r.hasApproved)
    .map((r) => ({ number: r.pr.number, title: r.pr.title, author: r.pr.user?.login || '' }))
