...
export async function checkMergeReadiness(args = {}, deps = {}) {
  ...
  if (!filesRes?.ok) {
    ...
    checks.push({ name: 'migrations', status: hasMigrations ? 'pass' : 'pass', detail: hasMigrations ? 'Database migrations detected — rollback plan required' : 'No database migrations' })
  }
