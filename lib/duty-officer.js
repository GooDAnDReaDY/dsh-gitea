/**
 * Repository duty officer: read-only snapshot of repository activity —
 * new PRs, unreviewed PRs, stale issues — with idempotent event IDs.
 * All writes require explicit policy/approval.
 */
...
export async function collectEvents(args = {}, deps = {}, now = new Date()) {
  ...
  const activePulls = pulls.slice(0, 20)
  ...
  const reviewResults = await Promise.all(
    activePulls.map(async (pr) => {
      ...
      return { pr, approved: reviews.some((r) => r.state === 'APPROVED') }
    }),
  )

  for (const { pr, approved } of reviewResults) {
    if (!approved) {
...
export async function buildDutyReport(args = {}, deps = {}, now = new Date()) {
  ...
  const actions = events.map((e) => {
    if (e.kind === 'new-pr') return { action: 'review', target: `#${e.number}`, reason: 'new PR' }
    if (e.kind === 'needs-review') return { action: 'review', target: `#${e.number}`, reason: 'PR awaiting approval' }
    if (e.kind === 'stale-issue') return { action: 'triage', target: `#${e.number}`, reason: `stale ${e.days} days` }
