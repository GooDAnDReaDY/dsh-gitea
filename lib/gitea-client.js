...
export class GiteaClient {
  constructor({ baseUrl, token, fetchImpl = fetch, timeoutMs = 30_000, retries = 1, retryDelayMs = 300, cacheTtlMs = 15_000 } = {}) {
  ...
  async request(method, path, { query, body, bypassCache = false } = {}) {
    ...
    if (isGet && this.cacheTtlMs > 0 && this.cache.has(cacheKey)) {
      ...
      if (!bypassCache && now - cachedEntry.at < this.cacheTtlMs) {
        return JSON.parse(JSON.stringify(cachedEntry.res))
      }
    }
    ...
    const headers = {
      Accept: (arguments[2] && arguments[2].accept) || 'application/json',
    ...
    const attempt = async () => {
      try {
        ...
        let data
        const isDiffOrText = (arguments[2] && arguments[2].accept && arguments[2].accept.includes('diff'))
        // #231: Gitea answers some endpoints (merge is one) with an empty body or
        // 204. Parsing those as JSON throws, so state the empty case explicitly
        // instead of relying on the catch below.
        const contentLength = response.headers && response.headers.get ? response.headers.get('content-length') : null
        const emptyBody = response.status === 204 || contentLength === '0'
        try {
          if (emptyBody) {
            data = undefined
          } else if (isDiffOrText && typeof response.text === 'function') {
