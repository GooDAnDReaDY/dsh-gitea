...
export class GiteaClient {
  ...
  async request(method, path, { query, body, bypassCache = false } = {}) {
    ...
    if (isGet && !bypassCache && this.cacheTtlMs > 0 && this.cache.has(cacheKey)) {
      const cached = this.cache.get(cacheKey)
      if (now - cached.at < this.cacheTtlMs) {
        return JSON.parse(JSON.stringify(cached.res))
      }
    ...
    const attempt = async () => {
      try {
        ...
        try {
          if (isDiffOrText && typeof response.text === 'function') {
        ...
        if (isGet && this.cacheTtlMs > 0 && cacheKey) {
          this.cache.set(cacheKey, { at: Date.now(), res })
        }
        return res
      } catch (err) {
        return {
