...
export async function buildGitSnapshot({ repoDir, execFile } = {}) {
  ...
  try {
    const branch = (await readGit(execFile, dir, ['rev-parse', '--abbrev-ref', 'HEAD'])).stdout.trim()
    const head = (await readGit(execFile, dir, ['rev-parse', 'HEAD'])).stdout.trim()
    ...
    const graph = (await readGit(execFile, dir, ['log', '--oneline', '-8'])).stdout.trim()
    let remoteUrl = ''
    try { remoteUrl = (await readGit(execFile, dir, ['remote', 'get-url', 'origin'])).stdout.trim() } catch { /* origin may be missing */ }
    const dirty = porcelain.trim().length > 0
    let diff = ''
    if (dirty) {
      diff = (await readGit(execFile, dir, ['diff', 'HEAD'])).stdout
