...
export function resolveRepoDir(settings = {}, args = {}) {
  const repoDir = String(args.path || args.repoDir || settings.repoDir || '').trim()
  if (!repoDir) {
    return { ok: false, error: 'Set working copy path in Settings (repoDir) or pass path.' }
  }
...
export async function runWorktreeAction(action, args = {}, deps = {}) {
  ...
  const resolved = resolveRepoDir(deps.settings || {}, args)
...
export async function buildGitSnapshot({ repoDir, execFile } = {}) {
  ...
  if (!dir) return { ok: false, error: 'Set working copy path in Settings (repoDir).' }
  try {
