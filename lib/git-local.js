...
export async function buildGitSnapshot({ repoDir, execFile } = {}) {
  ...
  try {
    ...
    const graph = (await git(execFile, dir, ['log', '--oneline', '-8'])).stdout.trim()
    let remoteUrl = ''
    try { remoteUrl = (await git(execFile, dir, ['remote', 'get-url', 'origin'])).stdout.trim() } catch { /* origin may be missing */ }
    const dirty = porcelain.trim().length > 0
    let diff = ''
    if (dirty) {
      diff = (await git(execFile, dir, ['diff', 'HEAD'])).stdout
