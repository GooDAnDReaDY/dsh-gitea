export const TOOL_DEFS = [
  ...
  },
  {
    ...
    description: 'List comments for a Gitea/Forgejo issue (each comment includes mine: true/false).',
    parameters: {
      number: { type: 'number', required: true, description: 'Issue number.' },
  ...
  {
    name: 'gitea_issue_comment_delete',
    description: 'Delete a comment on a Gitea/Forgejo issue. Requires confirm: true (boolean). Only comments authored by the current token user can be deleted; without confirm: true returns a dry-run preview.',
    parameters: {
      comment_id: { type: 'number', required: true, description: 'Comment id to delete.' },
      confirm: { type: 'boolean', description: 'Must be true to permanently delete the comment. Without it, returns a dry-run preview.' },
      owner: { type: 'string', description: 'Repository owner.' },
      repo: { type: 'string', description: 'Repository name.' },
      instance: { type: 'string', description: 'Gitea instance identifier (for multi-instance setups).' },
    },
  },
  {
