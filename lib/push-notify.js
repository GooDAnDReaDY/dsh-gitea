/**
 * push-notify: evaluates whether webhook events require push notifications
 * (new PR, failed CI) and formats notification text.
 */
...
export function buildNotifyMessage(event = {}) {
  ...
  if (event.type === 'pull_request') {
    return `[Gitea] New PR #${event.number}: ${event.title || ''} (${at}) ${event.url || ''}`.trim()
  }
...
export async function pushNotify(event = {}, deps = {}) {
  ...
  try {
    ...
    return { ok: res.ok, data: { notified: res.ok } }
  } catch (e) {
    return { ok: false, data: { notified: false, error: String(e?.message || e) } }
  }
}
