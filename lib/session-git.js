...
export const MAX_SESSION_DIRS = 300
const dirs = new Map()
loadPersistedDirs()

function persistFile() {
  if (process.env.NODE_TEST_CONTEXT && !process.env.TEST_PERSIST_SESSION_GIT) return ''
...
function loadPersistedDirs() {
  ...
  try {
    ...
    pruneDirs(MAX_SESSION_DIRS)
  } catch { /* missing store is fine */ }
}

export function pruneDirs(max = MAX_SESSION_DIRS) {
  while (dirs.size > max) {
    const oldest = dirs.keys().next().value
    if (oldest !== undefined) dirs.delete(oldest)
    else break
  }
}
...
export function scheduleSavePersistedDirs(delayMs = 250) {
  const file = persistFile()
  if (!file) return
  if (saveTimer) return
  saveTimer = setTimeout(() => {
    saveTimer = null
    savePersistedDirsAsync()
  }, delayMs)
  if (saveTimer && typeof saveTimer.unref === 'function') saveTimer.unref()
}

export async function savePersistedDirsAsync() {
  const file = persistFile()
  if (!file) return
  if (isSaving) {
    needsResave = true
  ...
  try {
    ...
    await fs.promises.writeFile(file, JSON.stringify(Object.fromEntries(dirs)), 'utf8')
  } catch { /* never break a tool */ }
  finally {
    isSaving = false
    if (needsResave) {
      needsResave = false
      scheduleSavePersistedDirs(50)
    }
  }
}

export function flushPersistedDirsSync() {
  const file = persistFile()
  if (!file) return
  try {
    fs.mkdirSync(path.dirname(file), { recursive: true })
    fs.writeFileSync(file, JSON.stringify(Object.fromEntries(dirs)), 'utf8')
  } catch { /* never break a tool */ }
}
...
export function rememberSessionGitDir(sessionId, repoDir) {
  ...
  if (!id) return
  if (dirs.get(id) === dir) return // dirty check: 0ms I/O when unchanged
  dirs.set(id, dir)
  pruneDirs(MAX_SESSION_DIRS)
  scheduleSavePersistedDirs()
}
...
function rememberedSessionGitDir(sessionId) {
  const id = String(sessionId || '').trim()
  if (id && dirs.has(id)) return dirs.get(id)
  return ''
}

export function isForbiddenPath(dir) {
  if (!dir || typeof dir !== 'string') return true
  const norm = path.normalize(dir).replace(/\\/g, '/').toLowerCase()
  if (norm === '/' || norm === '' || norm === '.' || norm === '~' || /^[a-z]:\/?$/.test(norm)) return true
  ...
  if (/^[a-z]:\/(windows|program files|program files \(x86\)|programdata|recovery|system volume information)(\/|$)/.test(norm)) {
    return true
  }
  if (norm.includes('/../') || norm.endsWith('/..')) return true
  return false
}

export function resolveSessionGitDir({ cwd, sessionId } = {}) {
  const fromQuery = String(cwd || '').trim()
  if (fromQuery && !isForbiddenPath(fromQuery)) return fromQuery
  const remembered = rememberedSessionGitDir(sessionId)
  return (remembered && !isForbiddenPath(remembered)) ? remembered : ''
}

export async function selectChipRepoDir({ cwd, sessionId } = {}, isGitDir, recover) {
  const explicit = String(cwd || '').trim()
  if (explicit && !isForbiddenPath(explicit) && await isGitDir(explicit)) return explicit
...
function isJunkPath(dir) {
  ...
  if (isForbiddenPath(value)) return true
...
function resolveDir(dir, base = '') {
  ...
  const isPosix = value.startsWith('/') || base.startsWith('/')
  let resolved
  if (value.startsWith('/') || /^[A-Za-z]:[\\/]/.test(value) || path.isAbsolute(value)) {
    resolved = isPosix ? path.posix.normalize(value.replace(/\\/g, '/')) : path.normalize(value).replace(/\\/g, '/')
  } else if (base) {
    resolved = isPosix
      ? path.posix.normalize(path.posix.join(base.replace(/\\/g, '/'), value.replace(/\\/g, '/')))
      : path.normalize(path.join(base, value)).replace(/\\/g, '/')
  } else {
    resolved = value.replace(/\\/g, '/')
  }
  return isJunkPath(resolved) ? '' : resolved
}

function lastCdDir(command) {
...
function withParents(dir, rank) {
  const out = []
  let current = String(dir || '').replace(/\\/g, '/')
  let currentRank = rank
  for (let i = 0; i < 6; i += 1) {
    if (!current || isJunkPath(current)) break
    out.push({ dir: current, rank: currentRank })
    const isPosix = current.startsWith('/')
    const parent = isPosix ? path.posix.dirname(current) : path.dirname(current).replace(/\\/g, '/')
