...
export async function runHandler(name, args = {}, deps = {}) {
  ...
  switch (name) {
    ...
    case 'gitea_issue_get_comments':
      return wrap(await client.listIssueComments(owner, repo, asNumber(args.number), pickQuery(args, ['limit', 'page'])))
    ...
    case 'gitea_issue_comment_delete':
      return wrap(await client.deleteIssueComment(owner, repo, asNumber(args.id || args.comment_id)))
...
export function formatToolResult(name, value) {
  ...
  if (data && typeof data === 'object') {
    ...
    if (name === 'gitea_issue_comment_delete') return [{ type: 'text', text: 'Comment deleted.' }]
    if (name === 'gitea_pr_diff') {
      if (data.stat) {
        return [{ type: 'text', text: `PR #${data.number}: ${data.filesCount} files changed (+${data.additions}, -${data.deletions})` }]
      }
