...
export async function runHandler(name, args = {}, deps = {}) {
  ...
  switch (name) {
    ...
    case 'gitea_issue_get': {
      ...
      // Fetch comments separately — Gitea API returns comment count but not objects
      const comments = await client.listIssueComments(owner, repo, asNumber(args.number))
      if (comments?.ok) {
    ...
    if (data.number != null) {
      ...
      if (Array.isArray(data.comments) && data.comments.length > 0) {
        const comments = data.comments.map((c) => {
          const author = c?.user?.login || c?.login || 'unknown'
          const text = c?.body || ''
          return `**${author}:** ${text}`
        }).join('\n')
