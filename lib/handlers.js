...
const NO_REPO = new Set(['gitea_repo_search', 'gitea_flavor', 'gitea_whoami', 'gitea_issue_search', 'gitea_issue_lint', 'gitea_org_repos', 'gitea_org_members', 'gitea_user_search', 'gitea_org_list', 'gitea_org_teams', 'gitea_notifications', 'gitea_notifications_mark_read', 'gitea_ci_explain', 'gitea_pr_template_check', 'gitea_mirror_public', ...LOCAL_TOOLS])
...
export async function runHandler(name, args = {}, deps = {}) {
  ...
  switch (name) {
    ...
    case 'gitea_sprint_plan': {
      const { planSprint } = await import('./sprint-plan.js')
      return planSprint({ owner, repo }, deps)
    }
