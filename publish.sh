...
# (AGENTS.md, index.md, plans, design contract, superpowers, .gitea, deploy.sh).
...
GITHUB_URL="https://github.com/GooDAnDReaDY/dsh-gitea.git"
GITHUB_BRANCH="main"
...
case "$MODE" in
  ...
  publish)
    [ "${DSH_GITEA_PUBLISH:-}" = "yes" ] || fail "publication requires explicit owner approval: run with DSH_GITEA_PUBLISH=yes"
    [ -n "$VERSION" ] || fail "usage: DSH_GITEA_PUBLISH=yes bash publish.sh publish <ref> <version>"
    ...
    git -c credential.helper="!gh auth git-credential" fetch "$GITHUB_URL" "$GITHUB_BRANCH" >/dev/null 2>&1 \
      || fail "cannot read current public $GITHUB_BRANCH from GitHub"
...
internal documents (AGENTS.md, index.md, plans, design contract, superpowers, .gitea).")"
  ...
  *)
    fail "usage: bash publish.sh plan [ref] | DSH_GITEA_PUBLISH=yes bash publish.sh publish <ref> <version>"
    ;;
esac
