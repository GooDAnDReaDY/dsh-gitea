...
- Develop on MiniAI at `/mnt/external/Project/DEV/dhsplugins/dsh-gitea`. Git only through `git-cursor`.
...
Worktree after Task 1: `/mnt/external/Project/DEV/dhsplugins/dsh-gitea/.worktrees/feat-1-gitea-a/`
...
Use cursor token from `/mnt/external/Project/DEV/.gitea-agent-credentials.json`. Never print the token. If the repo exists, reuse it. Do not create a personal-namespace repo.

- [ ] **Step 2: One canonical clone**

```bash
export PATH=/home/vadim/.ssh/bin:/usr/bin:/bin
cd /mnt/external/Project/DEV/dhsplugins
test ! -e dsh-gitea
...
```bash
cd /mnt/external/Project/DEV/dhsplugins/dsh-gitea
...
MIT LICENSE copyright 2026 dsh-gitea contributors. AGENTS.md: three identity sites, git-cursor, no secrets, npm test, file: reinstall is remove+add. Copy spec+plan into docs/superpowers. Short index.md with test command and staging-only note.
...
- [ ] **Step 1: Failing vm test** (pattern from `dhsplugins/dsh-tts/test/client-factory.test.mjs`): source has `id: '@goodandready/dsh-gitea'`, `var module = { exports: {} }`, `return module.exports`, `inject: ['slots']`, `apply` is a function, no `192.168` `/opt/` `/mnt/`.
...
- [ ] **Step 2: Leak grep** on `lib/`, `test/`, `README.md`, `AGENTS.md` for `192.168`, `/opt/`, `/mnt/external`, tokens. `goodandready` allowed only in package name and intended GitHub URLs.

```bash
rg -n -i '192\.168|/opt/|/mnt/external' lib test README.md AGENTS.md
```
