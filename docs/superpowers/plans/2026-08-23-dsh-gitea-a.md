...
- [ ] **Step 1: Failing vm test** (pattern from `plugins/dsh-tts/test/client-factory.test.mjs`): source has `id: '@goodandready-private/dsh-gitea'`, `var module = { exports: {} }`, `return module.exports`, `inject: ['slots']`, `apply` is a function, no `192.168` `INSTALLED_PATH/` `DEV_SOURCE/`.
...
- [ ] **Step 2: Leak grep** on `lib/`, `test/`, `README.md`, `AGENTS.md` for `192.168`, `INSTALLED_PATH/`, `DEV_SOURCE`, tokens. `goodandready` allowed only in package name and intended GitHub URLs.
