...
**Tech Stack:** Node ESM, `node:test`, schemastery, `defineTool`, `credentialRef`, Cordis settings and webServer. Gitea API v1 with `Authorization: token`. Git on the dev host: `git-agent`.
...
| No B/C/the dev host ops | Global + 6 |
